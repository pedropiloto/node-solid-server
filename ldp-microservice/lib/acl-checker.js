...
class ACLChecker {
  ...
  static createFromLDPAndRequest (resource, ldp, req) {
    ...
    return new ACLChecker(resource, {
      ...
      // host: req.get('host'),
      fetch: fetchLocalOrRemote(ldp.resourceMapper, ldp.serverUri, ldp.solidIdUri),
 ...
 */
function fetchLocalOrRemote (mapper, serverUri, solidIdUri) {
  return async function fetch (url, graph = rdf.graph()) {
    // Convert the URL into a filename
    let body, path, contentType

    if (Url.parse(url).host.includes(Url.parse(serverUri).host) || Url.parse(url).host.includes(Url.parse(solidIdUri).host)) {
